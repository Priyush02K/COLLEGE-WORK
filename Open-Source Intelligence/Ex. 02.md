# Experiment No. 01 — Email Header Analysis and Information Gathering using theHarvester

## 1. Title

**Email Header Analysis and Information Gathering using OSINT Tools**

---

## 2. Aim

To perform email header analysis and use the **theHarvester** OSINT tool to collect publicly available information such as email addresses, subdomains, hosts, employee/person names, IP addresses, and publicly reported technical information from authorized sources.
...
- Understand the structure of an email and its header.
- Extract and interpret email header information.
- Identify mail servers and email routing information.
- Identify sender IP information when it is available.
- Analyze email timestamps and authentication results.
- Use **theHarvester** for passive OSINT information gathering.
- Collect publicly indexed email addresses, hosts, subdomains, URLs, and names.
- Understand the difference between passive OSINT and active reconnaissance.
- Document and verify OSINT findings.

---

## 4. Background

An email contains more technical information than what is normally displayed in an email application. The **email header** contains metadata generated while the message is created and transferred between mail servers.

Email headers can contain:
...
- Email client information, where available

OSINT investigators can use this information during:
...
> **Important:** The originating/sender IP address is not guaranteed to be present. Modern email services may hide or remove the user's original IP address.
...
An **email header** is a collection of technical metadata associated with an email message.

A simplified email structure is:
...
+--------------------------------------------------+
| Hello Student,                                  |
| Welcome to OSINT Lab.                           |
+--------------------------------------------------+
```

---

## 6. Important Email Header Fields

| Header Field | Description |
|---|---|
| `From` | Sender address shown in the message |
| `To` | Recipient address |
| `Subject` | Subject of the email |
| `Date` | Date and time associated with the message |
| `Message-ID` | Unique identifier of the message |
| `Received` | Records mail-server processing/routing information |
| `Return-Path` | Address used for bounced mail |
...
| `Content-Type` | Format/type of message content |

---

## 7. Why is the `Received` Header Important?

Mail servers normally add `Received` headers as a message passes through them.

Example:

```text
Received: from mail.example.edu
    by smtp.example.net
    Tue, 11 Aug 2026 10:30:00 +0530
```

Multiple `Received` entries can provide information about the route taken by the email.

### Important Note

Do not automatically assume that the first visible IP address is the sender's real device IP. Email providers and intermediate servers may hide, replace, or add IP information.

---

## 8. Email Header Analysis Procedure

### Step 1 — Obtain a Safe Sample Email

Use one of the following:

- An email you own.
- A sample email supplied by the instructor.
- An authorized institutional test email.

**Do not upload confidential or private emails to third-party analysis services without permission.**

### Step 2 — Display the Full Header

Depending on the email service, use options such as:
...
Copy the complete header for analysis.

### Step 3 — Identify Basic Metadata
...
### Step 4 — Analyze `Received` Headers

Look for:

- Sending/receiving mail servers
- Hostnames
- IP addresses, if exposed
- Timestamps
- Routing sequence

### Step 5 — Analyze Authentication
...
Record whether the relevant checks indicate:
...
Do not treat a single authentication result as proof that an email is legitimate. Interpret it together with other evidence.

### Step 6 — Record Findings

Use the observation table below.

---

## 9. Email Header Observation Table
...
| 4 | Message-ID | |
| 5 | Return-Path | |
| 6 | Sending Mail Server | |
| 7 | Receiving Mail Server | |
| 8 | IP Address (if available) | |
| 9 | SPF Result | |
| 10 | DKIM Result | |
| 11 | DMARC Result | |
| 12 | Email Client/User Agent (if available) | |
| 13 | Routing Information | |
| 14 | Geographic information, if inferable | |

> Geographic location inferred from an IP address is approximate and should not be treated as the exact physical location of the sender.
